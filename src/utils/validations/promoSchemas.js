...
const BasePromoSchema = z.object({
  ...
  validity_end_at: z.number().min(dayjs().unix(), "End date must be in the future"),
...
export const AddPromoSchema = BasePromoSchema.refine(validateCoverageCountries, {
  message: "At least one country must be selected for public promos",
  path: ["coverage_countries"],
});

export const UpdatePromoSchema = BasePromoSchema.extend({
  _id: z.string(),
