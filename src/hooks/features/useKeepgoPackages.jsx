...
import {
  removeKeepgoPackageFromList,
  setSelectedKeepgoPackageData,
  setKeepgoPackageMetaData,
  updateKeepgoPackageInList,
  addNewKeepgoPackageToList,
} from "../../features/packages/package-keepgo/packageKeepgoSlice";
import {
...
} from "../../features/packages/package-keepgo/packageKeepgoApi";
...
import {
  AddKeepgoPackageSchema,
  UpdateKeepgoPackageSchema,
} from "../../utils/validations/packageKeepgoSchemas";
...
// ** KeepgoPackage List **
export const useGetKeepgoPackages = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { dataList, meta, selectedData } = useSelector(
    (state) => state.packageKeepgo
  );
  ...
  const { isLoading, isFetching, isError, error } =
    useGetAllKeepgoPackagesQuery(apiParams, {
  ...
  const [packageId, setKeepgoPackageId] = useState(null);
  const updatePageMeta = (value) => dispatch(setKeepgoPackageMetaData(value));
  const handleSetSelectedKeepgoPackage = (data) =>
    dispatch(setSelectedKeepgoPackageData(data));
  const [deleteKeepgoPackage, { isLoading: deleteLoading }] =
    useDeleteKeepgoPackageMutation();
  const [updatingKeepgoPackages, setUpdatingKeepgoPackages] = useState({});
  const [updateKeepgoPackageKeepgoPackage] = useUpdateKeepgoPackageMutation();
  ...
  useEffect(() => {
    ...
    // dispatch(setKeepgoPackageMetaData({ ...meta, current_page: 1 }));
  ...
  const handleDelete = async () => {
    if (!packageId) {
      errorNotify("KeepgoPackage ID is required.");
      return false;
    }

    try {
      const response = await deleteKeepgoPackage({ id: packageId }).unwrap();
      if (response?.success) {
        dispatch(removeKeepgoPackageFromList({ _id: packageId }));
  ...
  const handleNavigate = (item) => {
    dispatch(setSelectedKeepgoPackageData(item));
    navigate("/keepgo-package-edit");
  };

  const handleOpenAddKeepgoPackageModal = () => {
    navigate("/keepgo-package-add", {
      state: {
        type: "add",
      },
    });
  ...
  const handleStatusChange = async (packageId, newStatus) => {
    ...
    setUpdatingKeepgoPackages((prev) => ({ ...prev, [packageId]: true }));

    try {
      const result = await updateKeepgoPackageKeepgoPackage({
        id: packageId,
        data: { status: apiStatus },
      }).unwrap();
      dispatch(
        updateKeepgoPackageInList({ ...result?.data, _id: result?.data._id })
      );
    } catch (error) {
      errorNotify(error?.data?.message || "Failed to change status");
    } finally {
      setUpdatingKeepgoPackages((prev) => {
        const newState = { ...prev };
  ...
  return {
    ...
    setSearchKeyword,
    selectedData,
    handleSetSelectedKeepgoPackage,
    filterKey,
    setFilterKey,
    setKeepgoPackageId,
    handleNavigate,
...
export const useAddKeepgoPackage = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  ...
  const [addKeepgoPackage] = useAddKeepgoPackageMutation();
  ...
  useEffect(() => {
    // Form validation
    const result = AddKeepgoPackageSchema.safeParse(formData);
    if (result.success) {
      setErrors({});
    }
  ...
  const validateForm = useCallback(() => {
    ...
    const result = AddKeepgoPackageSchema.safeParse(dataToValidate);
    if (!result.success) {
      const newErrors = {};
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const validatedData = AddKeepgoPackageSchema.parse({
        ...formData,
      ...
      const response = await addKeepgoPackage({ data: validatedData }).unwrap();

      if (response?.success) {
        setIsModalVisible(true);
        dispatch(addNewKeepgoPackageToList(response.data));
      }
    } catch (error) {
...
export const useUpdateKeepgoPackage = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { selectedData } = useSelector((state) => state.packageKeepgo);
  ...
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isModalVisible, setIsModalVisible] = useState(false);
  const [updatePackage] = useUpdateKeepgoPackageMutation();
  ...
  useEffect(() => {
    // Form validation
    const result = UpdateKeepgoPackageSchema.safeParse(formData);
    if (result.success) {
      setErrors({});
    }
  ...
  const validateForm = useCallback(() => {
    ...
    const result = UpdateKeepgoPackageSchema.safeParse(numericData);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const validatedData = UpdateKeepgoPackageSchema.parse({
        ...formData,
        data_plan_in_mb: Number(formData.data_plan_in_mb),
        bonus_data_plan_in_mb: Number(formData.bonus_data_plan_in_mb) || 0,
        validity: {
